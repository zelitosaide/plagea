...
export default function SamplesPage() {
  ...
  const filteredSamples = samples.filter((sample) => {
    ...
    const matchesType =
      filterType === "all" || sample.sampleType === filterType;
    const matchesProject =
      filterProject === "all" || sample.project === filterProject;
    const matchesFreezer =
      !freezerFilter || sample.freezerId === freezerFilter.split(":")[0];
  ...
  const getFreezerName = (freezerId: string) => {
    return freezerId
      ? freezerId.split(":")[1] || "Unknown Freezer"
      : "All Freezers";
  };
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-600 mt-2">
                Pesquisar, filtrar e gerenciar amostras de laboratório
              </p>
              {freezerFilter && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <Select value={filterProject} onValueChange={handleProjectChange}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="COVID-19 Study">
                    Estudo sobre COVID-19
                  </SelectItem>
                  <SelectItem value="Diabetes Research">
                    Pesquisa sobre Diabetes
                  </SelectItem>
                  <SelectItem value="Genetic Analysis">
                    Análise Genética
                  </SelectItem>
            ...
            <CardTitle>
              ...
              {freezerFilter && (
                <span className="text-sm font-normal text-gray-600 ml-2">
                  in {getFreezerName(freezerFilter)}
                </span>
              )}
            </CardTitle>
            <CardDescription>
                ...
                {currentSamples.length === 0 ? (
                  <div className="text-center py-8">
                    <p className="text-gray-500">
                      {freezerFilter
                        ? `Nenhuma amostra encontrada em ${getFreezerName(
                            freezerFilter
                          )}`
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                            <div>
                              ...
                              <p>
                                ...
                                {sample.freezerCode}-{sample.shelf}-{sample.box}
                                -{sample.position}
                              </p>
                            </div>
                            <div>
                              ...
                              <p>
                                <span className="font-medium">Hora Lab:</span>{" "}
                                {sample.labEntryTime || "N/A"}
                              </p>
                              <p>
                                <span className="font-medium">
                                  Data de Validade:
...
LOCALIZAÇÃO:
Congelador: ${sample.freezerCode}
Prateleira: ${sample.shelf}
Box: ${sample.box}
Posição: ${sample.position}
Localização Completa: ${sample.freezerCode}-${sample.shelf}-${sample.box}-${
                                  sample.position
                                }

DATAS E HORÁRIOS:
Data de Entrada: ${sample.entryDate}
Hora de Entrada no Laboratório: ${sample.labEntryTime || "Não informado"}
Hora de Armazenamento: ${sample.freezerStorageTime || "Não informado"}
Data de Validade: ${sample.expiryDate}

OBSERVAÇÕES:
${sample.notes || "Nenhuma observação registrada"}`
                              )
                            }
                          >
